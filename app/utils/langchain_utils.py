...
# set up parser
parser_client = LlamaParse(result_type="markdown", api_key=LLAMA_PARSER_API)
...
class Loaders:
    ...
    def __init__(
        self, loader_type: str = "llama_parser", webloader_type: str = "reader"
    ) -> None:
        ...
        try:
            if self.loader_type == "text" and file_path is not None:
                ...
                documents = SimpleDirectoryReader(
                    input_files=[file_path], file_extractor=Loaders.file_extractor
                ).load_data()
