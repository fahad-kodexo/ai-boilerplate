...
async def create_chat_user(request: Request, user_chat: UserChat):
    try:
        user = await ChatHistory().insert_chat_user(user_chat.email)
...
async def get_sessions_by_email(request: Request):
    try:
        ...
        user_session = await ChatHistory().get_user_sessions(email)
