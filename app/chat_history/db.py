...
class ChatHistory(MongoManager):
    async def insert_chat_history(self,user_id:str,query,
        ...
        async with ChatHistory() as db:
            ...
            update_data = {
                "$push": {
                    "chat_history": {
                        "$each": [user_doc, assistant_doc]
                    }
                }
            }
            return await user_collection.find_one_and_update({"user_id":user_id},update_data,upsert=True)
